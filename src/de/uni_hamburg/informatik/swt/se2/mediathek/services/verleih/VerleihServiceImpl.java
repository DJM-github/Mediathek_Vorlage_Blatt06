        ...
        VerleihService
{
            ...
            List<Verleihkarte> initialBestand)
    {
        ...
        assert initialBestand != null : "Vorbedingung verletzt: initialBestandVer  != null";
            ...
            throws ProtokollierException
    {
        ...
        for (Medium medium : medien)
        {
            ...
            if(zeigeVormerkkartenFuer(medium).size()>0)
            {
                LinkedList<Vormerkkarte> Liste = _vormerkkarten.get(medium);
                Liste.remove(0);
                _vormerkkarten.put(medium, Liste);
            }
        }
    ...
    public boolean istVormerkenFuerMediumMoeglich(Kunde kunde, Medium medium)
    {
        assert kunde != null : "Vorbedingung verletzt: kunde ist null";
        assert medium != null : "Vorbedingung verletzt: medium ist null";
        
        List<Vormerkkarte> vormerkkartenListe = zeigeVormerkkartenFuer(medium);
        for (Vormerkkarte vormerkkarte : vormerkkartenListe)
        {
            if(vormerkkarte.getVormerker()==kunde)
            {
                return false;
            }
        }
        
        if(vormerkkartenListe.size() < 3)
        {
            return true;
        }
        return false;
    ...
    public boolean istVormerkenFuerAlleMoeglich(Kunde kunde, List<Medium> medien)
    {
        for (Medium medium : medien)
        {
            if(!istVormerkenFuerMediumMoeglich(kunde, medium))
            {
                return false;
            }
        }
    ...
    public void merkeAusgewaehlteMedienVor(Kunde kunde, List<Medium> medien, Datum datum)
    {
        assert kundeImBestand(kunde) : "Vorbedingung verletzt: kundeImBestand(kunde)";
        assert datum != null : "Vorbedingung verletzt: ausleihDatum != null";
        assert istVormerkenFuerAlleMoeglich(kunde, medien) : "Vorbedingung verletzt: Vormerken nicht möglich";
        
        for (Medium medium : medien)
        {
            Vormerkkarte vormerkkarte = new Vormerkkarte(kunde, medium, datum);
            LinkedList<Vormerkkarte> Liste = _vormerkkarten.get(medium);
//            if(Liste == null)
//            {
//                LinkedList<Vormerkkarte> neueListe = new LinkedList<Vormerkkarte>();
//                neueListe.add(vormerkkarte);
//            }
//            else
//            {
            Liste.add(vormerkkarte);
//            }
            _vormerkkarten.put(medium, Liste);
    ...
    public void storniereVormerkung(Vormerkkarte vormerkkarte)
    {
        LinkedList<Vormerkkarte> Liste = _vormerkkarten.get(vormerkkarte.getMedium());
        Liste.remove(vormerkkarte);
        _vormerkkarten.put(vormerkkarte.getMedium(), Liste);
    }
    
    public LinkedList<Vormerkkarte> zeigeVormerkkartenFuer(Medium medium)
    {     
        return _vormerkkarten.get(medium);
    }
    
    public LinkedList<Vormerkkarte> zeigeVormerkkarten()
    {
        LinkedList<Vormerkkarte> Liste = new LinkedList<Vormerkkarte>();
        
        for (LinkedList<Vormerkkarte> vormerkkarten : _vormerkkarten.values())
        {
