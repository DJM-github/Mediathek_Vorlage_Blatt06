...
public class AusleihWerkzeug
{
    ...
    private boolean istAusleihenMoeglich()
    {
        ...
        if(ausleiheMoeglich){
            ...
            {
                List<Vormerkkarte> Liste = _verleihService.zeigeVormerkkartenFuer(medium);
                if(Liste.size()!=0 && _verleihService.zeigeVormerkkartenFuer(medium).get(0).getVormerker()!=kunde)
                {
                    return false;
                }
            }
